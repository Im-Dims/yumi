...
export async function before(m) {
  ...
  if (!user.autolevelup) return !0
	...
	if (before !== user.level) {
	  let ini_txt = `Selamat @${m.sender.replace(/@.+/g, '')}, Kamu Telah Naik Level!\n\n*-* *Level up* : *${before}* -> *${user.level}*\n*-* *Role up* : *${user.role}*`.trim()
	    try {
	      let image, data, pp
	        try {
	        ...
	        await sock.reply(global.media.scm, `*[ System Notifications ]*\n\n${m.pushName} baru saja naik level!!`, null)
	      })
		} catch {
